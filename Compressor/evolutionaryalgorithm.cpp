...
int EvolutionaryAlgorithm::compress(VoronoiDiagram * outputDiagram,
	...
	int selectionCount = POPULATION_SIZE * SELECTION_RATE;
	int crossoverCount = selectionCount * CROSSOVER_RATE;
	int mutationCount = selectionCount - crossoverCount;
	...
	while (true) {
		...
		for (int i = 0; i < crossoverCount; ++i) {
			...
			while (!addedNewMember) {
				int firstParentIndex = Utils::generateRandom(selectedPopSize - 1);
				int secondParentIndex = Utils::generateRandom(selectedPopSize - 1);
		...
		int mutationPopSize = selectedPopSize;
		for (int i = 0; i < mutationCount; ++i) {
			int memberToMutateIndex = Utils::generateRandom(mutationPopSize - 1);
			...
			population.insert(population.begin() + mutationPopSize, memberToMutate);
			populationFitness.insert(populationFitness.begin() + mutationPopSize, memberToMutateFitness);
